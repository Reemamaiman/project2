package com.example.employees;

import org.springframework.http.ResponseEntity;
import org.springframework.validation.Errors;
import org.springframework.web.bind.annotation.*;

import javax.validation.Valid;
import java.util.ArrayList;

public class CategoryController {

        @RestController
        @RequestMapping("/api/v1/category")
        public class ProductController{


            private ArrayList<ArrayList<Category>> categoryList = new ArrayList<>();

            @GetMapping("/category")
            public ResponseEntity getUsersController() {

                return ResponseEntity.status(200).body(categoryList);
            }

            @PostMapping("/category")
            public ResponseEntity addUser(@RequestBody @Valid User user, Errors errors) {
                if (errors.hasErrors()) {
                    String message = errors.getFieldError().getDefaultMessage();
                    return ResponseEntity.status(400).body(new ApiResponse(message, 400));

                }
                return ResponseEntity.status(201).body(new ApiResponse("new product add !", 201));
            }

            @PutMapping("/category/{index}")
            public ResponseEntity updateProduct(@RequestBody @Valid Category
                                                       category
                    , @PathVariable int index, Errors errors) {
                if (errors.hasErrors()) {
                    String message = errors.getFieldError().getDefaultMessage();
                    return ResponseEntity.status(400).body(new ApiResponse(message, 400));
                }
                if (index >= categoryList.size()) {
                    return ResponseEntity.status(400).body(new ApiResponse("Invalid index !", 400));
                }
                categoryList.set(index,categoryList);
                return ResponseEntity.status(201).body(new ApiResponse("category updated !", 201));

            }

            @DeleteMapping("/merchant/{index}")
            public ResponseEntity deleteUser(@PathVariable int index) {
                if (index >= categoryList.size()) {
                    return ResponseEntity.status(400).body(new ApiResponse("category deleted !", 400));
                }

               categoryList.remove(index);
                return ResponseEntity.status(200).body(new ApiResponse("category updated !", 200));

            }
        }


    }


